...
export interface IFindTransactionByCompanyNamePresenter<
  NotFoundType,
  SuccessType,
  FunctionalErrorType,
> {
...
export class FindTransactionByCompanyNameUseCase<
  NotFoundType,
  SuccessType,
  FunctionalErrorType,
> implements
    IFindTransactionByCompanyNameUseCase<
      SuccessType,
      FunctionalErrorType,
      NotFoundType
    >
{
  constructor(
    ...
    private presenter: IFindTransactionByCompanyNamePresenter<
      NotFoundType,
      SuccessType,
      FunctionalErrorType
    >,
  ...
  async execute(
    companyName: string,
  ): Promise<NotFoundType | SuccessType | FunctionalErrorType> {
    try {
      const merchant =
        await this.merchantRepository.findByCompanyName(companyName);
      if (!merchant) {
        return await this.presenter.notFound();
      }
      const transactions =
        await this.transactionRepository.findAllTransactionsByMerchantId(
          merchant.id,
        );
