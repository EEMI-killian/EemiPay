...
export class TransactionArggregate implements ITransactionArggregate {
    ...
    addOperation({type, amount, currency, customerCardInfo, merchantIban}: {
        type: "CAPTURE" | "REFUND";
        amount: number;
        currency: string;
        customerCardInfo: cardInfo;
        ...
        if(type == "REFUND"){
            ...
            const initialCapture = this.operations.find(op => op.type === "CAPTURE" && (op.status === "COMPLETED"));
            if(!initialCapture) {
                return { error: "No capture operation found to refund against." }
            }
            const totalRefunded = this.operations
                .filter(op => op.type === "REFUND" && (op.status === "COMPLETED" || op.status === "PENDING"))
                .reduce((sum, op) => sum + op.amount, 0);
            if(totalRefunded + amount > initialCapture.amount) {
                return { error: "Refund amount exceeds the initial capture amount." }
            }
            this.operations.push({ id: `operation-${uuidv4()}`, type, amount, currency, customerCardInfo , createdAt: new Date(), updatedAt: new Date(), merchantIban: merchantIban , status: "PENDING" });
